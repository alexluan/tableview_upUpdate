...
+ (YALSunnyRefreshControl*)attachToScrollView:(UIScrollView *)scrollView
    ...
    [refreshControl.scrollView addObserver:refreshControl forKeyPath:@"contentOffset" options:NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld context:nil];//contentoffset通知
    refreshControl.target = target;
    refreshControl.action = refreshAction;//方法指针
    [scrollView setDelegate:refreshControl];//tableview代理方法给自己
...
#pragma clang diagnostic ignored "-Warc-performSelector-leaks"
        [self.target performSelector:self.action withObject:nil];//执行代理方法
#pragma clang diagnostic pop
        self.forbidSunSet = YES;
    }
    
    if(!self.forbidSunSet){//创建刷新动画
...
-(void)endRefreshing{
    ...
    self.forbidOffsetChanges = NO;//停止变化
